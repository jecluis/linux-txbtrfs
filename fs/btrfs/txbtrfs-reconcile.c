 ...
 */
static struct symexec_entry *
__list_tree_lookup_file(struct rb_root * root, u64 parent, u64 ino,
		struct qstr * name)
{
	struct list_head * lst;
	struct symexec_entry * entry;
	...
	lst = __list_tree_get(root, parent);
	if (IS_ERR(lst))
		return ERR_CAST(lst);
	else if (!lst)
		return NULL;
	...
	list_for_each_entry(entry, lst, list) {
		if (((ino != 0) && (entry->ino != ino)) || (entry->file->name.hash != name->hash)
...
static struct symexec_entry *
symexec_rem_lookup_file(struct symexec_ctl * symexec, u64 parent, u64 ino,
		struct qstr * name)
{
	if (!symexec || !name)
		return ERR_PTR(-EINVAL);
	return __list_tree_lookup_file(&symexec->rem, parent, ino, name);
}

static int symexec_rem_match_file(struct symexec_ctl * symexec, u64 parent,
		u64 ino, struct qstr * name, u8 * origin)
{
	...
	entry = symexec_rem_lookup_file(symexec, parent, ino, name);
	if (IS_ERR(entry)) {
		...
		struct btrfs_acid_log_create * entry)
{
	...
	if (err < 0) {
		BTRFS_SYM_DBG("parent ino: %llu; entry parent ino: %llu\n",
				parent_ino, entry->file.parent_location.objectid);
		return err;
	}
	...
	if (err) {
		BTRFS_SYM_DBG("objid: %llu; parent ino: %llu; entry parent ino: %llu\n",
				objectid, parent_ino, entry->file.parent_location.objectid);
		return err;
	}
	...
	err = symexec_dir_put(symexec, parent_ino, symexec_entry);
	if (err < 0) {
		...
		struct btrfs_acid_log_entry * entry)
{
	...
	if (err < 0) {
		BTRFS_SYM_DBG("parent ino: %llu; entry parent ino: %llu\n",
				parent_ino, file->parent_location.objectid);
		return err;
	}
	...
	if (err) {
		err = symexec_dir_remove(symexec, parent_ino, ino, &file->name);
		if (err < 0) {
			BTRFS_SYM_DBG("DIR-REMOVE > parent: %llu; ino: %llu; name: %.*s\n",
					parent_ino, ino, file->name.len, file->name.name);
			return err;
		}
		...
		rem_entry = __symexec_create_entry(parent_ino, file,
				entry, ORIGIN_GLOBAL);
		if (IS_ERR(rem_entry)) {
		...
		struct btrfs_acid_log_entry * entry)
{
	...
	err = __symexec_get_mapped(symexec,
			file->parent_location.objectid, &parent_ino);
	if (err < 0) {
		BTRFS_SYM_DBG("parent ino: %llu; entry parent ino: %llu\n",
				parent_ino, file->parent_location.objectid);
		return err;
	...
	err = symexec_dir_put(symexec, parent_ino, dir_entry);
	if (err < 0) {
		...
		struct btrfs_acid_log_unlink * entry)
{
	...
	if (err < 0) {
		...
		rem_entry = __symexec_create_entry(parent, &entry->file,
				(struct btrfs_acid_log_entry *) entry, ORIGIN_LOCAL);
		if (IS_ERR(rem_entry)) {
			BTRFS_SYM_DBG("CREATE-ENTRY > parent: %llu, ino: %llu, name: %.*s\n",
					parent, ino, name->len, name->name);
